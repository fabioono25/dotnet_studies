...
To update the database use this command from the Web project folder (replace `Company.FirstTest` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Company.FirstTest.Infrastructure/Company.FirstTest.Infrastructure.csproj -s Company.FirstTest.Web.csproj
```
