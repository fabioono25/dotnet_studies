...
To update the database use this command from the Web project folder (replace `Test.CleanArchitecture` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Test.CleanArchitecture.Infrastructure/Test.CleanArchitecture.Infrastructure.csproj -s Test.CleanArchitecture.Web.csproj
```
